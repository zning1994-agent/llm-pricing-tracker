...
const LastUpdated: React.FC<LastUpdatedProps> = ({
  ...
  return (
    <div 
      className={`inline-flex items-center gap-2 font-medium ${sizeClasses[size]} ${getFreshnessColor()}`}
      title={formatAbsoluteTime(lastUpdated)}
    >
      {showIcon && (
        <>
          <span 
            className={`inline-block w-2 h-2 rounded-full ${getFreshnessDot()} ${lastUpdated ? 'animate-pulse' : ''}`}
          />
